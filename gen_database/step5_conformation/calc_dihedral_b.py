...
for index, row in df.iterrows():
    if '_b' in row['File']:
        ...
        atom_4 = eval(row['CO2H_atoms'])[1] if len(eval(row['CO2H_atoms'])) > 1 else ''
...
output_df.to_csv('dihedral_b_temp.csv', index=False)
df1 = pd.read_csv('dihedral_b_temp.csv')
...
def process_file(input_file, ring_atoms, idx1, idx2, idx3, idx4, angle_deg):
    ...
    try:
        ...
        output_file = input_file.replace('.xyz', f'_conf{angle_deg}.xyz')
...
def main():
    ...
    for row in rows:
        ...
        try:
            ...
            is_coplanar = row['is_coplanar'].lower() == 'true'
        ...
        mol = AllChem.MolFromXYZFile(input_file)
        if mol is None:
            results.append({'File': file_id, 'status': 'skipped', 'reason': 'Failed to load molecule'})
            continue
        ...
        if is_coplanar:
            # Check the initial dihedral angle
            initial_angle = rdMolTransforms.GetDihedralDeg(mol.GetConformer(), idx1-1, idx2-1, idx3-1, idx4-1)
            if abs(initial_angle) < 10 or abs(initial_angle) > 350:
                angle_deg = 180
            elif abs(initial_angle - 180) < 10 or abs(initial_angle - 180) > 350:
                angle_deg = 0
            ...
            # Create two configurations, one with angle 0 and one with angle 180
            success_0, reason_0 = process_file(input_file, ring_atoms, idx1, idx2, idx3, idx4, 0)
            success_180, reason_180 = process_file(input_file, ring_atoms, idx1, idx2, idx3, idx4, 180)
            if success_0 and success_180:
                results.append({'File': file_id, 'status': 'processed', 'reason': None})
            else:
                reason = reason_0 if not success_0 else reason_180
                results.append({'File': file_id, 'status': 'skipped', 'reason': reason})
    ...
    print(f"Processing complete. Results saved to {result_csv}")
...
combined_df = pd.merge(df1, df2, on='File')
combined_df.to_csv('dihedral_b.csv', index=False)

files_to_delete = ['output.csv','dihedral_b_temp.csv','combined_output.csv','processing_results.csv']
