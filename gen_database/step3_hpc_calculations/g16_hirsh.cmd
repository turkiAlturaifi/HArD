#!/usr/bin/env bash
#SBATCH --job-name=g16
#SBATCH --output=g16_sp_submit_hirsh.out
...
for parent_folder in {1..250}; do
    ...
    for folder in */; do
        pushd "$folder"
        cd opt_outputs
        python -m aqme --qprep --files "*.log" --qm_input "m062x/6-31+G(d) scrf=(smd,solvent=water) pop=Hirshfeld" --mem 16GB --nproc 8 --suffix SP_hirsh --program gaussian
        mv QCALC sp_hirsh
        mv sp_hirsh ../
        cd ../sp_hirsh
        cp ../g16_h2p.cmd .
